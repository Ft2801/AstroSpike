...
void AstroSpikeCanvas::render(QPainter& p, float scale, const QPointF& offset) {
    ...
    for (const auto& star : m_stars) {
        ...
        if (star.radius > m_config.maxStarSize) continue;
...
AstroSpikeDialog::AstroSpikeDialog(QWidget* parent)
    : QDialog(parent), m_buffer(nullptr)
{
...
void AstroSpikeDialog::setImageBuffer(ImageBuffer* buffer) {
    ...
    // Update Canvas Image
    m_canvas->setImage(m_buffer->getDisplayImage(ImageBuffer::Display_Linear));
...
void AstroSpikeDialog::runDetection() {
    ...
    m_thread = new StarDetectionThread(*m_buffer, this);
...
void AstroSpikeDialog::applyToDocument() {
    ...
    QImage fullImg = m_buffer->getDisplayImage(ImageBuffer::Display_Linear);
    ...
    m_buffer->lockWrite();
    std::vector<float>& data = m_buffer->data();
    int w = m_buffer->width();
    int c = m_buffer->channels();
    ...
    // Clear stars so they don't render twice (once in baked pixels, once by dynamic renderer)
    m_canvas->setStars({});
    m_allStars.clear();
    m_history.clear();
    m_historyIndex = -1;
    updateHistoryButtons();
    m_statusLabel->setText(tr("Spikes applied to image buffer."));
}
