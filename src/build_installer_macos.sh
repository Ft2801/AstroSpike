...
echo "==========================================="
echo " AstroSpike Installer Builder (macOS)"
echo "==========================================="
echo ""
...
# Verify distribution
verify_dir "dist/AstroSpike.app" "Distribution" || exit 1
...
rm -f "installer_output/AstroSpike_Setup_"*.dmg
echo "  - Cleaned"
...
DMG_NAME="AstroSpike_Setup_${VERSION}.dmg"
DMG_PATH="installer_output/$DMG_NAME"
DMG_TEMP="installer_output/AstroSpike_temp.dmg"
...
# Copy app to staging
cp -R "dist/AstroSpike.app" "$STAGING_DIR/"
...
if [ -n "$CREATE_DMG" ]; then
    ...
    if [ -f "src/images/AstroSpike.icns" ]; then
        VOL_ICON="--volicon src/images/AstroSpike.icns"
    fi
    
    create-dmg \
        --volname "AstroSpike $VERSION" \
        $VOL_ICON \
        --window-pos 200 120 \
        --window-size 600 400 \
        --icon-size 100 \
    ...
    hdiutil create -volname "AstroSpike $VERSION" \
